...
function getKeyFlow(i) {
    const blocks = textToBlocks(i.toString());
...
function encrypt(text, key) {
    const inputKey = key;
    const bytes = Buffer.from(inputKey, 'utf8');
    ...
    const textToBit = textToBitBlocks(text);
...
function decrypt(encryptedText, key) {
    ...
    const textArr = Array.from(encryptedText).map(Number);
    ...
    return bitToText(bitsDecrypted);
}
